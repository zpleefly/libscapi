...
namespace semihonestot {
	...
	void OTExtensionReceiver::HashValues(CBitVector& T, int ctr, int processedOTs)
	{
		...
		const int array_size = numhashiters * SHA1_BYTES;
		std::vector<BYTE> hash_buf(array_size);
		SHA_BUFFER sha_buf;
		SHA_CTX sha;

		for (int hash_ctr, i = ctr; i < ctr + processedOTs; i++, Tptr += OTEXT_BLOCK_SIZE_BYTES)
		{
			sha_buf.data = &hash_buf[0];
			...
			m_nRet.SetBits(&hash_buf[0], i * m_nBitLength, m_nBitLength);
		}
	}
	...
	BOOL OTExtensionReceiver::verifyOT(int NumOTs)
	{
		...
		vector<BYTE> tempXc(bytelen);
		vector<BYTE> tempRet(bytelen);
		BYTE resp;
		for (int i = 0; i < NumOTs;)
		{
			...
			for (int j = 0; j < OTsPerIteration && i < NumOTs; j++, i++)
			{
				...
				Xc->GetBits(&tempXc[0], j*m_nBitLength, m_nBitLength);
				m_nRet.GetBits(&tempRet[0], i*m_nBitLength, m_nBitLength);
	...
	void OTExtensionSender::MaskInputs(CBitVector& Q, CBitVector* SndBuf, int ctr, int processedOTs)
	{
		...
		SHA_CTX sha, shatmp;
		SHA_BUFFER sha_buf;
		vector<BYTE> hash_buf(numhashiters * SHA1_BYTES);
		BYTE* Qptr = Q.GetArr();
		...
		for (int i = ctr, j = 0; j < processedOTs; i++, j++)
		{
			...
			for (int u = 0; u < m_nSndVals; u++)
			{
				...
				if (m_bProtocol == G_OT)
				{
					SndBuf[u].SetBits(&hash_buf[0], j* m_nBitLength, m_nBitLength);
				...
				else if (m_bProtocol == C_OT)
				{
					if (u == 0)
					{
						m_vValues[0].SetBits(&hash_buf[0], i*m_nBitLength, m_nBitLength);
					}
					else
					{
						SndBuf[0].SetBits(&hash_buf[0], j*m_nBitLength, m_nBitLength);
					}
				}
				else //R_OT
				{
					m_vValues[u].SetBits(&hash_buf[0], i*m_nBitLength, m_nBitLength);
				}
			}
		}
		if (m_bProtocol == G_OT)
